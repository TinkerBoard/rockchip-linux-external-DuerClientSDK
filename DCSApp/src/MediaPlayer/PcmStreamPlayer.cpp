...
PcmStreamPlayer::PcmStreamPlayer(const std::string& audio_dev) : m_executor(NULL),
                                                                 m_alsaCtl(NULL),
                                                                 m_pcmBuffer(NULL),
                                                                 m_currentLen(0L),
...
void PcmStreamPlayer::pushData(const char *data, unsigned int len) {
    if (!m_pushingFlag) {
        return;
    }
...
void PcmStreamPlayer::ttsEnd() {
    m_pushingFlag = false;
}

void PcmStreamPlayer::ttsStop() {
    m_alsaCtl->aslaAbort();
    m_alsaCtl->alsaClear();
    m_pushingFlag = false;
    m_stopFlag = true;
...
void PcmStreamPlayer::executePlaybackStarted() {
...
void PcmStreamPlayer::executePlaybackStopped() {
    size_t size = m_listeners.size();
    for (size_t i = 0; i < size; ++i) {
        if (NULL != m_listeners[i]) {
...
void* PcmStreamPlayer::playFunc(void *arg) {
    PcmStreamPlayer *player = (PcmStreamPlayer *) arg;
...
void PcmStreamPlayer::play_stream(unsigned int channels,
                            const void *buffer,
