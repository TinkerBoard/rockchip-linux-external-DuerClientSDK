...
Mp3StreamPlayer::Mp3StreamPlayer(const std::string &audio_dev) : m_executor(nullptr),
                                                                 m_alsaCtl(nullptr),
...
Mp3StreamPlayer::~Mp3StreamPlayer() {
    m_aliveFlag = false;
...
void Mp3StreamPlayer::registerListener(TtsPlayerListener *listener) {
    m_listeners.push_back(listener);
}

void Mp3StreamPlayer::ttsPlay() {
    m_removeHeadFlag = false;
...
void Mp3StreamPlayer::pushData(const char *data, unsigned int len) {
    m_logCtlFlag = !m_logCtlFlag;
    if (m_pushingFlag && len >= 0) {
        if (m_logCtlFlag) {
            APP_INFO("push data(%d) to Mp3StreamPlayer.", len);
        }
        m_streamPool.pushStream(data, len);
    }
}

void Mp3StreamPlayer::ttsEnd() {
    m_pushingFlag = false;
}

void Mp3StreamPlayer::ttsStop() {
    m_alsaCtl->aslaAbort();
    m_alsaCtl->alsaClear();
    m_pushingFlag = false;
    m_stopFlag = true;
...
void Mp3StreamPlayer::executePlaybackStarted() {
...
void Mp3StreamPlayer::executePlaybackStopped() {
    size_t size = m_listeners.size();
    for (size_t i = 0; i < size; ++i) {
        if (nullptr != m_listeners[i]) {
...
void* Mp3StreamPlayer::playFunc(void *arg) {
    auto player = (Mp3StreamPlayer *)arg;
...
void Mp3StreamPlayer::play_stream(unsigned int channels,
                            const void *buffer,
...
int Mp3StreamPlayer::readPacket(void *opaque, uint8_t *buf, int buf_size) {
    auto instance = (Mp3StreamPlayer *)opaque;
    while (true) {
...
void Mp3StreamPlayer::ffmpegLogOutput(void *ptr,
                                int level,
                                const char *fmt,
                                va_list vl) {
